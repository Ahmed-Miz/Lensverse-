...
export class DataCard implements OnInit {
  ...
  ngOnInit() {
    this.route.paramMap.subscribe((params) => {
      ...
      if (id) {
        this.article = this.posts.find((a) => a.id == +id);
      }
    });

    const fullText = this.article.content;

    const rawSections: string[] = fullText.split('## ');

    this.sections = rawSections.map((section: string): Section => {
    ...
    this.filterByCategory(this.article.category);
  }

  filteredArticles = [...this.posts];

  filterByCategory(category: any) {
    if (category === category) {
      this.filteredArticles = this.posts.filter((art) => art.category === category);
      this.filteredArticles = this.filteredArticles.slice(0, 3);
